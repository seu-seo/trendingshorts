...
──────────────     ──────────────      ─────────────      ──────────────     ────────────────
LLM 대화형          플랫폼·카테고리       닉네임 입력(필수)    카테고리 필터링      YouTube SSE 라이벌
4문답              ·연령대 선택          + Gemini 페르소나   ER% 정렬           + AI 2-step 제작
                   8개 카테고리          카드 표시           바텀시트 → 만들기    (generate→conti)
```
...
- **챗봇 온보딩** — LLM 대화형 4문답 → 취향 설정(플랫폼·카테고리 8종·연령대) → Gemini 페르소나
- **닉네임 로그인** — PersonaScreen 최상단 필수 입력 → `localStorage('user_profile')` 저장
- **트렌드** — 온보딩 카테고리 기반 필터링 + ER% 정렬 / 카드 탭 → 바텀시트(저장/만들기)
- **라이벌 크리에이터** — YouTube SSE 3단계 파이프라인 (검색→AI분석→Vision점수화)
- **제작(Production)** — 의도 입력 → AI 2-step: `/api/generate`(3종 대본) → `/api/conti`(4컷) + 캐싱
- **저장/마이페이지** — 프로필(이름·페르소나·카테고리) + 트렌드·크리에이터·대본·콘티 표시
