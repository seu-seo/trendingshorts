...
```
   앱 시작
      ↓
   온보딩 완료?
   ├── NO  → 온보딩 설문 (지은 설문 기반 7문항 + 앱 사용 목적 선택)
   │              ↓ 저장 완료
   └── YES ↓
         앱 사용 목적?
         ├── 트렌드 확인 / 인플루언서 추천  →  [대시보드 탭] 랜딩
         └── 제작 도움                    →  [추천·제작 탭] 랜딩

   ① 대시보드 탭                     ② 추천·제작 탭
   ──────────────                   ───────────────
   카테고리 탭 (페르소나 자동 선택)   영상 방향 설문 (분위기/길이/특징)
        ↓                                 ↓
   카테고리 TOP 3 영상               [소재·컨셉 추천받기] 버튼
   (채널 정보 포함)                       ↓  Gemini 호출
        ↓  Gemini 분석               영상 소재·컨셉 추천 3가지
   주요 키워드 분석                        ↓  Gemini 호출
                                    대본 3종 + 콘티 생성
```
...
지은 설문 기반 7문항 + 신규 "앱 사용 목적" 질문으로 페르소나를 도출합니다.

**설문 항목:**
1. 주로 활동하는 플랫폼 (YouTube / TikTok / Instagram)
2. 채널 카테고리
3. 크리에이터 경력 (슬라이더)
4. 목표 (팔로워 증가 / 수익화 / 브랜드 / 팬덤)
5. 콘텐츠 스타일 키워드 (다중 선택)
6. 가장 힘든 점 (주관식)
7. 업로드 빈도 (슬라이더)
8. **앱 사용 목적** (신규): 트렌드 확인 / 인플루언서 추천 / 제작 도움

앱 사용 목적에 따라 첫 랜딩 탭이 결정됩니다.

### 6.2 대시보드 탭 (`/`)

**카테고리 탭:** 페르소나 카테고리 기반 자동 선택, 터치로 전환 가능.

**카테고리 TOP 3 영상:**
- 플랫폼별 상위 3개 영상 카드 (조회수·좋아요·업로드 시각·해시태그)
- 채널(인플루언서) 정보:
  - YouTube: `channelId` → `channels.list` API ✅
  - TikTok: `authorMeta.followerCount` 포함 여부 확인 필요 ⚠️
  - Instagram: 별도 profile scraper 필요 여부 확인 필요 ⚠️
- [더보기] 터치 시 나머지 영상 펼침
...
- Q2. 영상 길이 (쇼츠 / 1분 내외 / 긴 영상)
- Q3. 강조하고 싶은 본인만의 특징
- 페르소나 + 설문 응답 → 추천 프롬프트 인풋 생성

**소재·컨셉 추천 (Gemini):**
- [소재·컨셉 추천받기] 버튼 클릭 시에만 Gemini 호출
- 트렌드 TOP + 키워드 + 페르소나 종합, 맞춤 소재 3가지 제안

**대본 + 콘티 생성 (Gemini):**
- 대본 3종: ① 정보형 ② 감성형 ③ 유머형 (HOOK + BODY + CTA)
- 콘티 생성: 장면 구성·자막·시각 요소 — 확인 필요 ⚠️
...
**Gemini 호출 3회:**
1. `/api/keywords` — 카테고리 키워드 분석 (대시보드)
2. `/api/recommend` — 소재·컨셉 추천 (추천·제작 탭)
3. `/api/generate` — 대본+콘티 생성 (기존 연동, 프롬프트 개선)
